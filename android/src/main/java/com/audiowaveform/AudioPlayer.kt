...
class AudioPlayer(
    ...
    private fun handleAudioFocusChange(focusChange: Int) {
        Log.d("TAG", "handleAudioFocusChange: focusChange${ focusChange}")
        when (focusChange) {
            AudioManager.AUDIOFOCUS_GAIN -> {
                Log.d("TAG", "handleAudioFocusChange: ${player.playWhenReady && !player.isPlaying} ${player.playWhenReady }${player.isPlaying }")
            ...
            AudioManager.AUDIOFOCUS_LOSS_TRANSIENT -> {
                ...
                Log.d("TAG", "handleAudioFocusChange: ${player.isPlaying}")
                if (player.isPlaying) {
                    player.pause()
                }
            }
            AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK -> {
                // Temporary loss of audio focus; lower the volume
                player.volume = 0.2f
            }
        }
    }
